...
:: Write .env — only create if missing; otherwise patch individual keys
if not exist ".env" (
    (
        ...
        echo.
        echo # Plex URL ^(container name for Docker, localhost for local dev^)
        echo PLEX_URL=http://musicstream-plex:32400
        echo.
        echo # Spotify token cache path
    ...
    echo [INFO] .env exists — patching missing/updated keys only.
...
:: ============================================================
:: STEP 4/9 - Spotify OAuth (generate spotify_token.json)
:: ============================================================
echo [STEP 4/9] Spotify OAuth authentication...
echo.

if exist "spotify_token.json" (
    echo [OK]   spotify_token.json already exists - skipping OAuth.
    echo        Delete it and re-run setup.bat to re-authenticate.
) else (
    echo [INFO] A browser window will open for Spotify login.
    echo        Log in and click Allow - the token will be saved automatically.
    echo        This is a one-time step. The daemon uses the saved token forever.
    echo.
    ...
    if not exist "spotify_token.json" (
        echo [ERROR] spotify_token.json was not created. Authentication may have failed.
        exit /b 1
    )
...
:: ============================================================
:: Subroutine: set or update a key in .env
:: Usage: call :patch_env KEY VALUE
:: - If KEY exists in .env, replaces the line
:: - If KEY is missing, appends it
:: ============================================================
:patch_env
set "_PKEY=%~1"
set "_PVAL=%~2"
...
if %errorlevel% equ 0 (
    :: Key exists — replace the line using PowerShell
