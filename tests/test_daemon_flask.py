...
class TestHealthLogic:
    def test_db_track_count_is_used(self):
        import src.daemon as dm
        with patch("src.daemon._get_db_track_count", return_value=999):
...
class TestBackupLogic:
    def test_backup_returns_path_on_success(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".sql", delete=False) as f:
            ...
            tmp_path = f.name
        try:
            with patch("src.daemon.db_backup", return_value=tmp_path):
                path = dm.db_backup()
    ...
    def test_backup_returns_none_on_failure(self):
        import src.daemon as dm
        with patch("src.daemon.db_backup", return_value=None):
            result = dm.db_backup()
